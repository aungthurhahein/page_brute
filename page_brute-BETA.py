...
def main():
    ...
    argument_parser = argparse.ArgumentParser(description="Checks pages in pagefiles for YARA-based rule matches."
                                                          " Useful to identify forensic artifacts within Windows-based page files and"
                                                          " characterize blocks based on regular expressions.")
            ...
            if not is_block_null(raw_page):
                ...
                for matches in authoritative_rules.match(data=raw_page):
                    if invert == True:
                        ...
                        if matches.rule not in page_list:
    ...
    # write page id with yara rule names with its corresponding data block
    o2 = open(SCANNAME+"_pageblocks", 'w')
    page_list2 = []
    scan_name = []
    ...
    for k,v in page_list.iteritems():
        for m in v.split(';'):
            ...
            scan_name.append(k)
            o.write(k+'\t'+m+"\n")
    for k2,v2 in page_block.iteritems():
        hitrule = ""
